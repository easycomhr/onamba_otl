...
@section('page_title', 'Duyệt Nghỉ phép')
...
<h1 class="text-xl font-bold text-gray-800 uppercase mb-6 hidden md:block">Danh sách đơn nghỉ phép cần duyệt</h1>
...
<div class="mb-6 flex items-center text-gray-600">
    ...
    <span>Đang có <strong class="text-green-600">{{ $leaveRequests->where('status', 'pending')->count() }}</strong> đơn nghỉ phép chờ bạn xử lý.</span>
</div>
...
<form method="GET" action="{{ route('admin.approvals.leave.index') }}" class="mb-5 flex flex-wrap gap-3">
    ...
    <button type="submit" class="px-5 py-2.5 bg-green-600 text-white font-bold rounded-xl hover:bg-green-700 transition-colors text-[15px]">Lọc</button>
</form>
...
<div class="md:bg-white md:rounded-2xl md:shadow-sm md:border md:border-gray-200 overflow-hidden">
    <table class="w-full text-left border-collapse">
        ...
        <tr>
            <th class="px-6 py-4 font-semibold text-gray-500 uppercase tracking-wider text-sm">Mã đơn</th>
            <th class="px-6 py-4 font-semibold text-gray-500 uppercase tracking-wider text-sm">Nhân viên</th>
            <th class="px-6 py-4 font-semibold text-gray-500 uppercase tracking-wider text-sm">Loại phép</th>
            <th class="px-6 py-4 font-semibold text-gray-500 uppercase tracking-wider text-sm">Thời gian nghỉ</th>
            <th class="px-6 py-4 font-semibold text-gray-500 uppercase tracking-wider text-sm text-center">Số ngày</th>
        ...
        <tr class="block md:table-row bg-white rounded-2xl shadow-sm mb-5 md:mb-0 md:rounded-none md:shadow-none md:border-b border-gray-100 hover:bg-gray-50 transition-colors">
            <td class="flex justify-between md:table-cell px-5 py-3 md:px-6 md:py-4 border-b border-gray-50 md:border-none">
                <span class="md:hidden font-medium text-gray-500">Mã đơn:</span>
            ...
            <td class="flex justify-between md:table-cell px-5 py-3 md:px-6 md:py-4 border-b border-gray-50 md:border-none">
                <span class="md:hidden font-medium text-gray-500">Loại phép:</span>
            ...
            <td class="flex justify-between md:table-cell px-5 py-3 md:px-6 md:py-4 border-b border-gray-50 md:border-none">
                <span class="md:hidden font-medium text-gray-500">Thời gian:</span>
            ...
            <td class="flex justify-between md:table-cell px-5 py-3 md:px-6 md:py-4 border-b border-gray-50 md:border-none md:text-center">
                <span class="md:hidden font-medium text-gray-500">Số ngày:</span>
                <span class="inline-flex items-center justify-center px-3 py-1 bg-green-50 text-green-700 font-bold rounded-lg">{{ $row->days }} ngày</span>
            </td>
            <td class="block md:table-cell px-5 py-4 md:px-6 bg-gray-50/50 md:bg-transparent border-t md:border-none border-gray-100">
                ...
                <div class="grid grid-cols-3 gap-3 md:flex md:justify-center md:gap-2">
                    <a href="{{ route('admin.approvals.leave.show', $row->id) }}"
                       class="flex flex-col md:flex-row items-center justify-center py-2 md:px-3 border border-gray-300 md:border-transparent rounded-xl md:rounded-lg text-gray-600 hover:bg-gray-100 hover:text-green-600 transition-colors">
                    ...
                    <form method="POST" action="{{ route('admin.approvals.leave.approve', $row->id) }}">
                        @csrf
                        <button type="submit" class="w-full flex flex-col md:flex-row items-center justify-center py-2 md:px-4 bg-green-50 text-green-700 border border-green-200 hover:bg-green-600 hover:text-white rounded-xl md:rounded-lg transition-colors">
        ...
        <tr class="block md:table-row">
            <td colspan="6" class="px-6 py-10 text-center text-gray-400">Không có đơn nào.</td>
        </tr>
        @endforelse
    ...
    <div id="rejectCard" class="bg-white w-full max-w-md rounded-2xl shadow-2xl overflow-hidden transform scale-95 transition-transform duration-300">
        <div class="bg-red-50 px-6 py-4 border-b border-red-100 flex items-center justify-between">
            <div class="flex items-center text-red-700">
                ...
                <h3 class="text-lg font-bold uppercase">Từ chối đơn nghỉ phép</h3>
            </div>
...
<script>
    ...
    function openRejectModal(id) {
        document.getElementById('rejectForm').action = '/admin/approvals/leave/' + id + '/reject';
