...
@section('title', 'Báo cáo tăng ca')
@section('page_title', 'Báo cáo tăng ca')

@section('content')
<h1 class="text-2xl font-extrabold text-gray-900 uppercase tracking-tight mb-6 hidden md:block">Báo cáo tăng ca</h1>
...
<div class="space-y-6">
    ...
    @if($otRequests->isNotEmpty())
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
            <div class="bg-blue-50 px-5 py-4 border-b border-blue-100">
                <h2 class="text-[17px] font-bold text-blue-800 uppercase">Chi tiết tăng ca</h2>
            </div>
                ...
                <table class="table-auto w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50 text-gray-600 uppercase text-xs tracking-wide">
                            ...
                            <th class="px-4 py-3 text-left font-semibold">Ngày OT</th>
                            <th class="px-4 py-3 text-right font-semibold">Giờ Duyệt</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
                        @foreach($otRequests as $ot)
                            <tr class="hover:bg-gray-50 transition-colors {{ $loop->even ? 'bg-gray-50/50' : '' }}">
                                ...
                                <td class="px-4 py-3 text-gray-800 text-right font-semibold">{{ $ot->approved_hours }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
                ...
                <table class="table-auto w-full text-sm">
                    <thead>
                        <tr class="bg-gray-50 text-gray-600 uppercase text-xs tracking-wide">
                            <th class="px-4 py-3 text-left font-semibold">Tên NV</th>
                            <th class="px-4 py-3 text-left font-semibold">Phòng Ban</th>
                            <th class="px-4 py-3 text-right font-semibold">Tổng ngày OT</th>
                            <th class="px-4 py-3 text-right font-semibold">Tổng giờ OT</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-100">
    ...
    @else
        @if(request()->hasAny(['from_date', 'to_date', 'employee_id', 'department']))
            <div class="bg-yellow-50 border border-yellow-200 text-yellow-700 rounded-xl px-5 py-4 text-[15px] font-medium">
                Không tìm thấy dữ liệu tăng ca cho bộ lọc đã chọn.
            </div>
        @endif
    @endif
